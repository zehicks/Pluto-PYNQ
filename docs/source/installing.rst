...
.. code-block:: console
    ...
    usb start && load usb 0 0x2500000 system_top.bit.bin && fpga load 0 0x2500000 0xeb6fc && load usb 0 ${fit_load_address} image.ub_aa && load usb 0 0x241fbc0 image.ub_ab && setenv bootargs "console=ttyPS0,115200n8 root=/dev/sda rw rootfstype=ext4 mem=512M rootwait" && bootm ${fit_load_address}
